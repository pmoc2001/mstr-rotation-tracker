...
with tool:
    ...
    blended_y = ROT_FRAC * ((YIELDS['MSTY']+YIELDS['STRK']+YIELDS['STRF'])/3)
    denom     = price * growth * ROT_FRAC * blended_y
    req_sh    = target_inc / denom if denom>0 else np.nan
    ...
    def project(sh):
        ...
        yrs_post= LIFE_EXP - years_ret
        kept   *= np.exp(btc_ret*yrs_post)
        inc_tot = proj*eff*(m/100*YIELDS['MSTY']+s/100*YIELDS['STRK']+f/100*YIELDS['STRF'])*yrs_post
        return kept, inc_tot

    kn, in_now = project(shares)
    kr, in_req  = project(int(req_sh))
...
with docs:
    ...
    st.markdown("""
    - **Income Planning**: calculates shares needed for a target income.
    - **Bayesian Decision**: STH-SOPA based rotate-now vs later.
    - **Allocation Optimizer**: splits into MSTY/STRK/STRF.
    - **Comparison Table**: kept vs required outcomes.
    - **Timeline & Monte Carlo**: simple plots of path and average.
    """)
